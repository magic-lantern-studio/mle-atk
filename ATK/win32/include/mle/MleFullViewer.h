...
#ifndef MLE_FULLVIEWER_H
#define MLE_FULLVIEWER_H
...
class SOWIN_DLL_API MleFullViewer : public SoWinViewer {
  SOWIN_OBJECT_ABSTRACT_HEADER(MleFullViewer, SoWinViewer);
...
public:
  ...
  { SoWinViewer::setSeekMode(enable); }

  SbBool isSeekMode()
  { return SoWinViewer::isSeekMode(); }

protected:
  MleFullViewer(HWND parent,
                    const char * name,
                    SbBool embed,
                    BuildFlag flag,
                    Type type,
...
private:
  ...
  class MleFullViewerP * pimpl;

  friend class MleGuiFullViewerP;
  friend class MleFullViewerP;
