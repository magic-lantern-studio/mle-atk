...
// src/Inventor/Win/viewers/MleExaminerViewer.cpp.  Generated from MleGuiExaminerViewer.cpp.in by configure.
...
/*!
  \class MleExaminerViewer MleExaminerViewer.h Inventor/Win/viewers/MleExaminerViewer.h
  \brief The MleExaminerViewer class is a 3D-model examination viewer.
  ...
  MleExaminerViewer as a model viewer that loads Inventor and VRML
  files from disk and places them inside the viewer for the end-user
  to examine:

  \code
  #include <Inventor/Win/SoWin.h>
  #include <Inventor/Win/viewers/MleExaminerViewer.h>
  ...
  main(int argc, char ** argv)
  {
    ...
    // inspection.
    MleExaminerViewer * viewer = new MleExaminerViewer(mainwin);
  ...
  MleExaminerViewer has a convenient interface for repositioning
  ...
  The MleExaminerViewer provides a user decoration's button for
  ...
  \sa MleFlyViewer, MlePlaneViewer
*/
...
/*!
  \fn MleExaminerViewer::MleExaminerViewer(HWND parent, const char * name, SbBool embed, MleFullViewer::BuildFlag flag, SoWinViewer::Type type)
...
/*!
  \fn MleExaminerViewer::MleExaminerViewer(HWND parent, const char * name, SbBool embed, MleFullViewer::BuildFlag flag, SoWinViewer::Type type, SbBool build)
...
*/
void
MleExaminerViewer::setAnimationEnabled(const SbBool enable)
{
...
SbBool
MleExaminerViewer::isAnimationEnabled(void) const
{
...
void
MleExaminerViewer::stopAnimating(void)
{
  if (PRIVATE(this)->currentmode != MleGuiExaminerViewerP::SPINNING) {
#if SOWIN_DEBUG
    SoDebugError::postWarning("MleExaminerViewer::stopAnimating",
                              "not animating");
#endif // SOWIN_DEBUG
    return;
  }
  PRIVATE(this)->setMode(this->isViewing() ?
                         MleGuiExaminerViewerP::IDLE :
                         MleGuiExaminerViewerP::INTERACT);
}
...
SbBool
MleExaminerViewer::isAnimating(void) const
{
  return PRIVATE(this)->currentmode == MleGuiExaminerViewerP::SPINNING;
}
...
void
MleExaminerViewer::setFeedbackVisibility(const SbBool enable)
{
  if (enable == PRIVATE(this)->axiscrossEnabled) {
#ifdef SOWIN_EXTRA_DEBUG
    SoDebugError::postWarning("MleExaminerViewer::setFeedbackVisibility",
...
SbBool
MleExaminerViewer::isFeedbackVisible(void) const
{
...
*/
void
MleExaminerViewer::setFeedbackSize(const int size)
{
#if SOWIN_DEBUG
  if (size < 1) {
    SoDebugError::postWarning("MleExaminerViewer::setFeedbackSize",
...
int
MleExaminerViewer::getFeedbackSize(void) const
{
...
SbBool
MleExaminerViewer::processSoEvent(const SoEvent * const ev)
{
#if SOWIN_DEBUG && 0 // debug
  SoDebugError::postInfo("MleExaminerViewer::processSoEvent",
                          "[invoked], event '%s'",
  ...
  const MleGuiExaminerViewerP::ViewerMode currentmode = PRIVATE(this)->currentmode;
  MleGuiExaminerViewerP::ViewerMode newmode = currentmode;
  ...
  if (type.isDerivedFrom(SoMouseButtonEvent::getClassTypeId())) {
    ...
    case SoMouseButtonEvent::BUTTON1:
      ...
      if (press && (currentmode == MleGuiExaminerViewerP::SEEK_WAIT_MODE)) {
        newmode = MleGuiExaminerViewerP::SEEK_MODE;
  ...
  if (type.isDerivedFrom(SoLocation2Event::getClassTypeId())) {
    ...
    if (PRIVATE(this)->currentmode == MleGuiExaminerViewerP::ZOOMING) {
      PRIVATE(this)->zoomByCursor(posn, prevnormalized);
    }
    else if (PRIVATE(this)->currentmode == MleGuiExaminerViewerP::PANNING) {
      MleGuiFullViewerP::pan(this->getCamera(), this->getGLAspectRatio(),
                            PRIVATE(this)->panningplane, posn, prevnormalized);
    }
    else if (PRIVATE(this)->currentmode == MleGuiExaminerViewerP::DRAGGING) {
  ...
  switch (combo) {
  case 0:
    if (currentmode == MleGuiExaminerViewerP::SPINNING) { break; }
    newmode = MleGuiExaminerViewerP::IDLE;
    if ((currentmode == MleGuiExaminerViewerP::DRAGGING) &&
      ...
      if (stoptime.getValue() < 0.100) {
        ...
        if ((radians > 0.01f) && (deltatime < 0.300)) {
          newmode = MleGuiExaminerViewerP::SPINNING;
    ...
    break;
  case BUTTON1DOWN:
    newmode = MleGuiExaminerViewerP::DRAGGING;
    break;
  case BUTTON3DOWN:
  case CTRLDOWN|BUTTON1DOWN:
  case SHIFTDOWN|BUTTON1DOWN:
    newmode = MleGuiExaminerViewerP::PANNING;
    break;
  case BUTTON1DOWN|BUTTON3DOWN:
  case CTRLDOWN|BUTTON3DOWN:
  case CTRLDOWN|SHIFTDOWN|BUTTON1DOWN:
    newmode = MleGuiExaminerViewerP::ZOOMING;
    break;
  ...
  default:
    ...
    // anything.
    if ((currentmode != MleGuiExaminerViewerP::SEEK_WAIT_MODE) &&
        (currentmode != MleGuiExaminerViewerP::SEEK_MODE)) {
      newmode = MleGuiExaminerViewerP::IDLE;
    }
    break;
  }
...
// documented in superclass
void
MleExaminerViewer::setSeekMode(SbBool on)
{
  ...
  // MleFlyViewer and MlePlaneViewer, so migrate any changes.

#if SOWIN_DEBUG
  if (on == this->isSeekMode()) {
    SoDebugError::postWarning("MleExaminerViewer::setSeekMode",
  ...
  PRIVATE(this)->setMode(on ?
                         MleGuiExaminerViewerP::SEEK_WAIT_MODE :
                         (this->isViewing() ?
                          MleGuiExaminerViewerP::IDLE : MleGuiExaminerViewerP::INTERACT));
}
...
*/
void
MleExaminerViewer::setCursorEnabled(SbBool enable)
{
...
// Documented in superclass.
const char *
MleExaminerViewer::getDefaultWidgetName(void) const
{
  return "MleExaminerViewer";
}
...
// Documented in superclass.
const char *
MleExaminerViewer::getDefaultTitle(void) const
{
...
// Documented in superclass.
const char *
MleExaminerViewer::getDefaultIconTitle(void) const
{
  return "Examiner Viewer";
}
...
// upon spin.
void
MleExaminerViewer::actualRedraw(void)
{
...
// doc in super
void
MleExaminerViewer::afterRealizeHook(void)
{
...
void
MleExaminerViewer::leftWheelMotion(float value)
{
  if (this->isAnimating()) this->stopAnimating();
...
void
MleExaminerViewer::bottomWheelMotion(float value)
{
  if (this->isAnimating()) this->stopAnimating();
...
void
MleExaminerViewer::rightWheelMotion(float value)
{
...
void
MleExaminerViewer::setViewing(SbBool enable)
{
  if (!!this->isViewing() == !!enable) {
#if SOWIN_DEBUG
    SoDebugError::postWarning("MleExaminerViewer::setViewing",
...
// Remaining code is for the MleGuiExaminerViewerP "private
// implementation" class.

MleGuiExaminerViewerP::MleGuiExaminerViewerP(MleExaminerViewer * publ)
{
  PUBLIC(this) = publ;
}

MleGuiExaminerViewerP::~MleGuiExaminerViewerP()
{
}

void
MleGuiExaminerViewerP::genericConstructor(void)
{
  this->currentmode = MleGuiExaminerViewerP::IDLE;
...
void
MleGuiExaminerViewerP::genericDestructor(void)
{
...
float
MleGuiExaminerViewerP::rotXWheelMotion(float value, float oldvalue)
{
...
// "rotX" wheel into camera movement.
float
MleGuiExaminerViewerP::rotYWheelMotion(float value, float oldvalue)
{
  SoCamera * camera = PUBLIC(this)->getCamera();
  if (camera == NULL) return 0.0f; // can happen for empty scenegraph
...
void
MleGuiExaminerViewerP::setMode(const ViewerMode newmode)
{
  ...
  if (oldmode == ZOOMING) {
    ...
    SoDebugError::postInfo("MleExaminerViewerP::setMode",
...
void
MleGuiExaminerViewerP::drawAxisCross(void)
{
...
void
MleGuiExaminerViewerP::drawArrow(void)
{
  glBegin(GL_LINES);
...
void
MleGuiExaminerViewerP::reorientCamera(const SbRotation & rot)
{
  SoCamera * cam = PUBLIC(this)->getCamera();
  if (cam == NULL) return;
...
void
MleGuiExaminerViewerP::spin(const SbVec2f & pointerpos)
{
...
void
MleGuiExaminerViewerP::clearLog(void)
{
  this->log.historysize = 0;
}
...
// animation calculations.
void
MleGuiExaminerViewerP::addToLog(const SbVec2s pos, const SbTime time)
{
  ...
  if (this->log.historysize > 0 && pos == this->log.position[0]) {
    ...
    SoDebugError::postInfo("MleGuiExaminerViewerP::addToLog", "got position already!");
#endif // debug
    return;
  }
...
// the model.
void
MleGuiExaminerViewerP::setMotion3OnCamera(SbBool enable)
{
...
// the model.
SbBool
MleGuiExaminerViewerP::getMotion3OnCamera(void) const
{
...
void
MleGuiExaminerViewerP::setCursorRepresentation(int modearg)
{
  ...
  switch (modearg) {
  case MleGuiExaminerViewerP::INTERACT:
    PUBLIC(this)->setComponentCursor(SoWinCursor(SoWinCursor::DEFAULT));
    break;

  case MleGuiExaminerViewerP::IDLE:
  case MleGuiExaminerViewerP::DRAGGING:
  case MleGuiExaminerViewerP::SPINNING:
    PUBLIC(this)->setComponentCursor(SoWinCursor::getRotateCursor());
    break;

  case MleGuiExaminerViewerP::ZOOMING:
    PUBLIC(this)->setComponentCursor(SoWinCursor::getZoomCursor());
    break;

  case MleGuiExaminerViewerP::SEEK_MODE:
  case MleGuiExaminerViewerP::SEEK_WAIT_MODE:
    PUBLIC(this)->setComponentCursor(SoWinCursor(SoWinCursor::CROSSHAIR));
    break;

  case MleGuiExaminerViewerP::PANNING:
